## this function fills holes (0s) less than 10-hours within each storms The data provided is the 
...
# is defined within this function as 10

StormChunk_10hr <- function(PPT) {
  rain_sum1 <-
  ...
  rainhour1 <- PPT0 + PPT1 + PPT2 + PPT3 + PPT4 + PPT5 + PPT6 + PPT7 + PPT8 + PPT9 + PPT10
  ...
  for(i in 1:nrow(Met_data_raw2)) {
    ...
    RS <- rs1 + rs2 + rs3 + rs4 + rs5 + rs6 + rs7 + rs8 + rs9 + rs10 + rs11
    #print(RS)
    RS_1 <- as.numeric(RS == 11)
    #print(RS_1)
    rain_sum1[i] <- rs1
    rain_sum2[i] <- rs2
    rain_sum3[i] <- rs3
    ...
    R_Sum <- data.frame(Met_data_raw2, rain_sum1, rain_sum2, rain_sum3, rain_sum4, rain_sum5, rain_sum6, rain_sum7, rain_sum8, rain_sum9, rain_sum10, rain_sum11, storm_occurrence)
