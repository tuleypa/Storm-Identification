## this function fills holes (0s) less than 12-hours within each storms The data provided is the 
...
# is defined within this function as 12

StormChunk_12hr <- function(PPT) {
  rain_sum1 <-
  ...
  PPT12 <- as.numeric(as.character(lead(PPT11, 1)))
  rainhour1 <- PPT0 + PPT1 + PPT2 + PPT3 + PPT4 + PPT5 + PPT6 + PPT7 + PPT8 + PPT9 + PPT10 + PPT11 + PPT12
  ...
  for(i in 1:nrow(Met_data_raw2)) {
    ...
    R_Sum <- data.frame(Met_data_raw2, rain_sum1, rain_sum2, rain_sum3, rain_sum4, rain_sum5, rain_sum6, rain_sum7, rain_sum8, rain_sum9, rain_sum10, rain_sum11, rain_sum12, rain_sum13, storm_occurrence)
