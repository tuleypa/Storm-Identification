## this function fills holes (0s) less than 9-hours within each storms The data provided is the 
...
# is defined within this function as 9

StormChunk_9hr <- function(PPT) {
  rain_sum1 <-
  ...
  rainhour1 <- PPT0 + PPT1 + PPT2 + PPT3 + PPT4 + PPT5 + PPT6 + PPT7 + PPT8 + PPT9
  ...
  for(i in 1:nrow(Met_data_raw2)) {
    ...
    R_Sum <- data.frame(Met_data_raw2, rain_sum1, rain_sum2, rain_sum3, rain_sum4, rain_sum5, rain_sum6, rain_sum7, rain_sum8, rain_sum9, rain_sum10, storm_occurrence)
